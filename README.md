...
| `settings::{SettingsSheet, SettingsFile, SettingsUi, SettingSpec, SettingsResponse, FONT_SCALE}` | renderer-neutral | shared F2 and platform settings shortcuts, focus-restoring and wheel-owning central sheet, persistent Poolrooms actuator, grouped setting layout, the canonical 100/125/150% pre-layout font-scale row, configuration-fault presentation, reload action, and semantic witnesses | setting declarations and values, storage, validation, reload execution, application-owned unpromoted controls, and which controls also appear in context |
