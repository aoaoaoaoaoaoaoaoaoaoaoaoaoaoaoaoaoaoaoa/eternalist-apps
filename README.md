...
The [glossary](docs/glossary.md) owns crystallized application terms;
[architecture](docs/architecture.md) owns subsystem boundaries, and the
[design language](docs/design-language.md) owns composition law.

## Present Surface

| Surface | Coordinate | Owns | Leaves To The Application |
| --- | --- | --- | --- |
| `ProductIdentity` | renderer-neutral | one compile-time validated reverse-DNS identifier and display name, its slug, and its platform directory triple | the declaration itself, which belongs in the product's contract crate, and every product-specific spelling such as window copy or witness prefixes |
| `Capabilities` | renderer-neutral | the facts the host declares about one run: pointer, touch, keyboard, power-unconstrained, configuration, and retirement | every policy applied to a fact, at the place it is applied |
| `Ingress` | native | the operating-system entry token: bare on a desktop, or Android's `NativeActivity` with its application handle | nothing; products hand it to the host unchanged |
| `ApplicationPaths` | native | the config, data, local data, cache, state, and runtime directories derived from a product identity and the entry's private root, and their creation | every file and subdirectory beneath them |
| `WindowSpec`, `ResponsivenessSpec`, and `CloseDisposition` | native | initial window identity and geometry, ordinary frame-work budget, and close-request vocabulary | dynamic title, product close policy, X11 floating disposition, and every window beyond the sole host window |
| `NativeApp` | native | the explicit hooks consumed by the host, including product identity and release, drawing, water sealing, optional GPU resource registration, optional post-present settlement, one-shot reveal and exit signals, rendering-independent service deadlines, one-constant crash-report enrollment, and optional one-way observation | domain state, service meaning, workers, persistence projection, typed command consequences, tray behavior, and dialogs |
| `run` and `run_with` | native | one host for desktops and Android entered through an `Ingress`: one-window winit/egui/wgpu lifecycle, surface loss and resumption, suspension, memory-warning, and retirement hooks, optional pre-construction recovery boundary, Poolrooms-water composition, focus- and concealment-aware repaint authority, rendering-independent service wakeups, surface recovery, responsiveness spans and trace deadline, local crash-capsule recovery, explicit report consent, and optional post-present witness publication | construction policy and dependencies, product shutdown work, multi-window policy, and faults outside the recoverable Rust host boundary |
| `CrashReportSpec` | native | one bounded local capsule at the product's state directory, sanitized panic evidence carrying the product identifier, next-launch consent, and one short background delivery attempt | enrollment, server operation and its admitted product set, returned host or environmental errors, and every fault that cannot execute a Rust panic hook |
...
| `Inspector`, `InspectorResponse`, `inspector::WIDTH`, `Drawer`, and `DrawerResponse` | renderer-neutral | the Dock: optional fixed left-side control-surface geometry, F9 and a zero-layout hover-revealed boundary actuator, translated slide motion, vertical scrolling, application return value, and shared water forcing | panels, fold state, actions, and persistence |
| `commands::{CommandCanon, CommandSpec, CommandScope, CommandStatus, CommandDispatch, CommandButtonResponse, Shortcut, ShortcutKey, ShortcutModifiers, TextFocusPolicy, RepeatPolicy}` | renderer-neutral | validated typed command metadata, stable IDs, portable exact shortcuts, reserved-idiom interlocks, visible Alt mnemonics, context precedence, disabled refusal, text-focus and repeat ownership, generated buttons, and one effective binding projection | command enum and scope enum, dynamic availability, domain execution, feedback presentation, and keymap persistence |
| `command_guide::{CommandGuide, GuideGroup, GuideGesture}` | renderer-neutral | F1/question-mark discovery, focus-restoring modal help generated from the command canon, current/all pages, and universal keyboard and guide groups | every target-specific group in product language, scope names, command availability, and domain help beyond interaction guidance |
| `settings::{SettingsSheet, SettingsFile, SettingsUi, SettingSpec, SettingsResponse, FONT_SIZE}` | renderer-neutral | shared F2 and platform settings shortcuts, focus-restoring and wheel-owning central sheet, persistent Poolrooms actuator, grouped setting layout, the canonical Font Size row projecting the Brass Font scale before layout, configuration-fault presentation, reload action, and semantic witnesses | setting declarations and values, storage, validation, reload execution, application-owned unpromoted controls, and which controls also appear in context |
...
| `Cabinet`, `CabinetEntry`, `CabinetKey`, `CabinetAction`, `CabinetResponse`, `CabinetBerth`, `CabinetShelfBerth`, `CabinetShelf`, `CabinetEntryEdit`, and `CabinetShelfEdit` | renderer-neutral | globally unique entry identity, root and one-level shelf ordering, entry and shelf drag berths, shelf folds and naming, optional inline entry renaming and editor requests, shared Poolrooms body, and semantic targets | entry meaning, editor transactions, active-document policy, product commands, storage projection, and persistence timing |
